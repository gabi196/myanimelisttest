...
describe('Teste de validação de senha durante o login', () => {
  it('Deve validar a senha incorreta', () => {
      ...
      cy.get('.btn-recaptcha-submit').click()
       ...
       cy.get('.btn-profile-submit:nth-child(2)').click() // Substitua pelo seletor correto
       ...
       cy.url().should('include', 'https://myanimelist.net/mangalist') // Substitua pelo fragmento da URL da lista de animes

       // Ou, se preferir, verificando algum elemento ou texto específico que só aparece na página de lista de animes:
       cy.contains('ALL MANGA').should('be.visible') // Substitua pelo texto ou elemento específico
   })
  })
